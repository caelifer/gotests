...
// countSetBitsInByte counts set bits byte represented as uint8
func countSetBitsInByte(b uint8) int {
	switch b {
	case 0:
		return 0
...
// countSetBitsInUint32 counts bits in provided uint32 by
// summing bit counts of 4 bytes that comprises uint32
func countSetBitsInUint32(ui uint32) int {
	return countSetBitsInByte(uint8(ui>>24)) +
		countSetBitsInByte(uint8(ui>>16)) +
		countSetBitsInByte(uint8(ui>>8)) +
		countSetBitsInByte(uint8(ui))
}
